...
func TestRequestHeaderSetCookieWithSpecialChars(t *testing.T) {
	...
	cookie := h1.Peek(constant.HeaderCookie)
...
func TestResponseHeaderDefaultStatusCode(t *testing.T) {
	...
	if statusCode != constant.StatusOK {
		t.Fatalf("unexpected status code: %d. Expecting %d", statusCode, constant.StatusOK)
	}
}
...
func TestRequestHeaderDel(t *testing.T) {
	...
	h.Set("Foo-Bar", "baz")
	h.Set("aaa", "bbb")
	h.Set(constant.HeaderConnection, "keep-alive")
	h.Set("Content-Type", "aaa")
	h.Set(constant.HeaderHost, "aaabbb")
	...
	hv = h.Peek(constant.HeaderConnection)
	if len(hv) > 0 {
		t.Fatalf("non-zero value: %q", hv)
	}
	hv = h.Peek(constant.HeaderContentType)
	if len(hv) > 0 {
		t.Fatalf("non-zero value: %q", hv)
	}
	hv = h.Peek(constant.HeaderHost)
	if len(hv) > 0 {
		t.Fatalf("non-zero value: %q", hv)
	}
	hv = h.Peek(constant.HeaderUserAgent)
	if len(hv) > 0 {
		t.Fatalf("non-zero value: %q", hv)
	}
	hv = h.Peek(constant.HeaderContentLength)
	if len(hv) > 0 {
		t.Fatalf("non-zero value: %q", hv)
	}
	hv = h.Peek(constant.HeaderCookie)
	if len(hv) > 0 {
		t.Fatalf("non-zero value: %q", hv)
	}
...
func TestResponseHeaderDel(t *testing.T) {
	...
	h.Set("aaa", "bbb")
	h.Set(constant.HeaderConnection, "keep-alive")
	h.Set(constant.HeaderContentType, "aaa")
	h.Set(constant.HeaderServer, "aaabbb")
	h.Set(constant.HeaderContentLength, "1123")
	...
	hv = h.Peek(constant.HeaderContentType)
	if string(hv) != string(constant.DefaultContentType) {
		t.Fatalf("unexpected content-type: %q. Expecting %q", hv, constant.DefaultContentType)
	}
	hv = h.Peek(constant.HeaderServer)
	if len(hv) > 0 {
		t.Fatalf("non-zero value: %q", hv)
	}
	hv = h.Peek(constant.HeaderContentLength)
...
func TestBufferSnippet(t *testing.T) {
	...
	b := string(util.CreateFixedBytes(199))
	...
	b = string(util.CreateFixedBytes(400))
	bExpected = fmt.Sprintf("%q", b)
	testBufferSnippet(t, b, bExpected)
	for i := 0; i < 10; i++ {
		b += "sadfqwer"
...
func testResponseHeaderSetContentRange(t *testing.T, startPos, endPos, contentLength int, expectedV string) {
	...
	v := h.Peek(constant.HeaderContentRange)
	if string(v) != expectedV {
...
func testRequestHeaderHasAcceptEncoding(t *testing.T, ae, v string, resultExpected bool) {
	var h RequestHeader
	h.Set(constant.HeaderAcceptEncoding, ae)
...
func TestResponseHeaderCopyTo(t *testing.T) {
	...
	h.Set(constant.HeaderSetCookie, "foo=bar")
	h.Set(constant.HeaderContentType, "foobar")
	h.Set("AAA-BBB", "aaaa")
	...
	if !bytes.Equal(h1.Peek(constant.HeaderContentType), h.Peek(constant.HeaderContentType)) {
...
func TestRequestHeaderCopyTo(t *testing.T) {
	...
	h.Set(constant.HeaderCookie, "aa=bb; cc=dd")
	h.Set(constant.HeaderContentType, "foobar")
	h.Set(constant.HeaderHost, "aaaa")
	h.Set("aaaxxx", "123")

	var h1 RequestHeader
	h.CopyTo(&h1)
	if !bytes.Equal(h1.Peek("cookie"), h.Peek(constant.HeaderCookie)) {
		t.Fatalf("unexpected cookie after copying: %q. Expected %q", h1.Peek("cookie"), h.Peek("cookie"))
	}
	if !bytes.Equal(h1.Peek("content-type"), h.Peek(constant.HeaderContentType)) {
		t.Fatalf("unexpected content-type %q. Expected %q", h1.Peek("content-type"), h.Peek("content-type"))
	}
...
func TestRequestHeaderConnectionClose(t *testing.T) {
	...
	h.Set(constant.HeaderConnection, "close")
	h.Set(constant.HeaderHost, "foobar")
	if !h.ConnectionClose() {
	...
	if string(h1.Peek(constant.HeaderConnection)) != "close" {
...
func TestResponseHeaderSetCookie(t *testing.T) {
	...
	h.Set(constant.HeaderSetCookie, "aaaaa=bxx")
...
func TestResponseHeaderVisitAllInOrder(t *testing.T) {
	...
	order := []string{
		...
		constant.HeaderCookie,
		constant.HeaderHost,
		constant.HeaderUserAgent,
	}
	values := []string{
		"aa",
		"a=b",
...
func TestRequestHeaderCookie(t *testing.T) {
	...
	h.Set(constant.HeaderHost, "foobar.com")
...
func TestResponseHeaderCookieIssue4(t *testing.T) {
	...
	if string(h.Peek(constant.HeaderSetCookie)) != "foo=bar" {
		t.Fatalf("Unexpected Set-Cookie header %q. Expected %q", h.Peek(constant.HeaderSetCookie), "foo=bar")
	}
	cookieSeen := false
	h.VisitAll(func(key, value []byte) {
		switch string(key) {
...
func TestRequestHeaderCookieIssue313(t *testing.T) {
	...
	if string(h.Peek(constant.HeaderCookie)) != "foo=bar" {
		t.Fatalf("Unexpected Cookie header %q. Expected %q", h.Peek(constant.HeaderCookie), "foo=bar")
	}
	cookieSeen := false
	h.VisitAll(func(key, value []byte) {
		switch string(key) {
...
func TestRequestHeaderMethod(t *testing.T) {
	...
	// common http methods
	testRequestHeaderMethod(t, constant.MethodGet)
	testRequestHeaderMethod(t, constant.MethodPost)
	testRequestHeaderMethod(t, constant.MethodHead)
	testRequestHeaderMethod(t, constant.MethodDelete)
...
func TestRequestHeaderSetGet(t *testing.T) {
	...
	h.SetMethod(constant.MethodPost)
	h.Set("foo", "bar")
	h.Set("host", "12345")
	...
	expectRequestHeaderGet(t, h, constant.HeaderHost, "12345")
	expectRequestHeaderGet(t, h, constant.HeaderContentType, "aaa/bbb")
	expectRequestHeaderGet(t, h, constant.HeaderContentLength, "1234")
	expectRequestHeaderGet(t, h, "USER-AGent", "aaabbb")
	expectRequestHeaderGet(t, h, constant.HeaderReferer, "axcv")
	expectRequestHeaderGet(t, h, "baz", "xxxxx")
	expectRequestHeaderGet(t, h, constant.HeaderTransferEncoding, "")
	...
	expectRequestHeaderGet(t, &h1, constant.HeaderContentType, "aaa/bbb")
	expectRequestHeaderGet(t, &h1, constant.HeaderContentLength, "1234")
	expectRequestHeaderGet(t, &h1, "USER-AGent", "aaabbb")
	expectRequestHeaderGet(t, &h1, constant.HeaderReferer, "axcv")
	expectRequestHeaderGet(t, &h1, "baz", "xxxxx")
	expectRequestHeaderGet(t, &h1, constant.HeaderTransferEncoding, "")
	expectRequestHeaderGet(t, &h1, constant.HeaderConnection, "close")
	if !h1.ConnectionClose() {
		t.Fatalf("unset connection: close")
	}
}
...
func TestResponseHeaderSetGet(t *testing.T) {
	...
	h.Set("connection", "close")
	h.Set("content-length", "1234")
	h.Set(constant.HeaderServer, "aaaa")
	h.Set("baz", "xxxxx")
	h.Set(constant.HeaderTransferEncoding, "chunked")

	expectResponseHeaderGet(t, h, "Foo", "bar")
	expectResponseHeaderGet(t, h, constant.HeaderContentType, "aaa/bbb")
	expectResponseHeaderGet(t, h, constant.HeaderConnection, "close")
	expectResponseHeaderGet(t, h, constant.HeaderContentLength, "1234")
	expectResponseHeaderGet(t, h, "seRVer", "aaaa")
	expectResponseHeaderGet(t, h, "baz", "xxxxx")
	expectResponseHeaderGet(t, h, constant.HeaderTransferEncoding, "")
	...
	expectResponseHeaderGet(t, &h1, constant.HeaderContentType, "aaa/bbb")
	expectResponseHeaderGet(t, &h1, constant.HeaderConnection, "close")
...
func TestResponseHeaderReadSuccess(t *testing.T) {
	...
	// no content-type
	testResponseHeaderReadSuccess(t, h, "HTTP/1.1 400 OK\r\nContent-Length: 123\r\n\r\nfoiaaa",
		400, 123, string(constant.DefaultContentType), "foiaaa")
	...
	// no headers
	testResponseHeaderReadSuccess(t, h, "HTTP/1.1 200 OK\r\n\r\naaaabbb",
		200, -2, string(constant.DefaultContentType), "aaaabbb")
	if !h.IsHTTP11() {
...
func verifyResponseHeader(t *testing.T, h *ResponseHeader, expectedStatusCode, expectedContentLength int, expectedContentType string) {
	...
	if string(h.Peek(constant.HeaderContentType)) != expectedContentType {
		t.Fatalf("Unexpected content type %q. Expected %q", h.Peek(constant.HeaderContentType), expectedContentType)
	}
}
...
func verifyRequestHeader(t *testing.T, h *RequestHeader, expectedContentLength int,
	...
	if string(h.Peek(constant.HeaderHost)) != expectedHost {
		t.Fatalf("Unexpected host %q. Expected %q", h.Peek(constant.HeaderHost), expectedHost)
	}
	if string(h.Peek(constant.HeaderReferer)) != expectedReferer {
		t.Fatalf("Unexpected referer %q. Expected %q", h.Peek(constant.HeaderReferer), expectedReferer)
	}
	if string(h.Peek(constant.HeaderContentType)) != expectedContentType {
		t.Fatalf("Unexpected content-type %q. Expected %q", h.Peek(constant.HeaderContentType), expectedContentType)
	}
