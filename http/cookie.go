...
func (c *Cookie) AppendBytes(dst []byte) []byte {
	...
	if c.maxAge > 0 {
		dst = append(dst, ';', ' ')
		dst = append(dst, constant.StrCookieMaxAge...)
		dst = append(dst, '=')
		...
		dst = append(dst, ';', ' ')
		dst = append(dst, constant.StrCookieExpires...)
		dst = append(dst, '=')
		dst = append(dst, c.bufKV.value...)
	}
	if len(c.domain) > 0 {
		dst = appendCookiePart(dst, constant.StrCookieDomain, c.domain)
	}
	if len(c.path) > 0 {
		dst = appendCookiePart(dst, constant.StrCookiePath, c.path)
	}
	if c.httpOnly {
		dst = append(dst, ';', ' ')
		dst = append(dst, constant.StrCookieHTTPOnly...)
	}
	if c.secure {
		dst = append(dst, ';', ' ')
		dst = append(dst, constant.StrCookieSecure...)
	}
	switch c.sameSite {
	case CookieSameSiteDefaultMode:
		dst = append(dst, ';', ' ')
		dst = append(dst, constant.StrCookieSameSite...)
	case CookieSameSiteLaxMode:
		dst = append(dst, ';', ' ')
		dst = append(dst, constant.StrCookieSameSite...)
		dst = append(dst, '=')
		dst = append(dst, constant.StrCookieSameSiteLax...)
	case CookieSameSiteStrictMode:
		dst = append(dst, ';', ' ')
		dst = append(dst, constant.StrCookieSameSite...)
		dst = append(dst, '=')
		dst = append(dst, constant.StrCookieSameSiteStrict...)
	case CookieSameSiteNoneMode:
		dst = append(dst, ';', ' ')
		dst = append(dst, constant.StrCookieSameSite...)
		dst = append(dst, '=')
		dst = append(dst, constant.StrCookieSameSiteNone...)
	}
	return dst
}
...
func (c *Cookie) ParseBytes(src []byte) error {
	...
	for s.next(kv) {
		if len(kv.key) != 0 {
			...
			case 'm':
				if caseInsensitiveCompare(constant.StrCookieMaxAge, kv.key) {
			...
			case 'e': // "expires"
				if caseInsensitiveCompare(constant.StrCookieExpires, kv.key) {
					v := bytesconv.ToString(kv.value)
			...
			case 'd': // "domain"
				if caseInsensitiveCompare(constant.StrCookieDomain, kv.key) {
					c.domain = append(c.domain[:0], kv.value...)
				}

			case 'p': // "path"
				if caseInsensitiveCompare(constant.StrCookiePath, kv.key) {
					c.path = append(c.path[:0], kv.value...)
				}

			case 's': // "samesite"
				if caseInsensitiveCompare(constant.StrCookieSameSite, kv.key) {
					// Case insensitive switch on first char
					switch kv.value[0] | 0x20 {
					case 'l': // "lax"
						if caseInsensitiveCompare(constant.StrCookieSameSiteLax, kv.value) {
							c.sameSite = CookieSameSiteLaxMode
						}
					case 's': // "strict"
						if caseInsensitiveCompare(constant.StrCookieSameSiteStrict, kv.value) {
							c.sameSite = CookieSameSiteStrictMode
						}
					case 'n': // "none"
						if caseInsensitiveCompare(constant.StrCookieSameSiteNone, kv.value) {
			...
			switch kv.value[0] | 0x20 {
			case 'h': // "httponly"
				if caseInsensitiveCompare(constant.StrCookieHTTPOnly, kv.value) {
					c.httpOnly = true
				}

			case 's': // "secure"
				if caseInsensitiveCompare(constant.StrCookieSecure, kv.value) {
					c.secure = true
				} else if caseInsensitiveCompare(constant.StrCookieSameSite, kv.value) {
