...
func ServeFileBytesUncompressed(ctx *RequestCtx, path []byte) {
	ServeFileUncompressed(ctx, bytesconv.ToString(path))
}
...
func ServeFileUncompressed(ctx *RequestCtx, path string) {
	ctx.Request.Header.DelBytes(constant.StrAcceptEncoding)
	ServeFile(ctx, path)
}
...
func ServeFileBytes(ctx *RequestCtx, path []byte) {
	ServeFile(ctx, bytesconv.ToString(path))
}
...
const FSCompressedFileSuffix = ".simo.gz"
...
func (r *bigFileReader) WriteTo(w io.Writer) (int64, error) {
	...
	// slow path
	return util.CopyZeroAlloc(w, r.r)
}
...
func (h *fsHandler) handleRequest(ctx *RequestCtx) {
	...
	mustCompress := false
	fileCache := h.cache
	byteRange := ctx.Request.Header.peek(constant.StrRange)
	if len(byteRange) == 0 && h.compress && ctx.Request.Header.HasAcceptEncodingBytes(constant.StrGzip) {
		mustCompress = true
	...
	if ff.compressed {
		hdr.SetCanonical(constant.StrContentEncoding, constant.StrGzip)
	}

	statusCode := constant.StatusOK
	contentLength := ff.contentLength
	if h.acceptByteRange {
		hdr.SetCanonical(constant.StrAcceptRanges, constant.StrBytes)
		if len(byteRange) > 0 {
			...
			statusCode = constant.StatusPartialContent
		}
	}

	hdr.SetCanonical(constant.StrLastModified, ff.lastModifiedStr)
	if !ctx.IsHead() {
...
func ParseByteRange(byteRange []byte, contentLength int) (startPos, endPos int, err error) {
	b := byteRange
	if !bytes.HasPrefix(b, constant.StrBytes) {
		return 0, 0, fmt.Errorf("unsupported range units: %q. Expecting %q", byteRange, constant.StrBytes)
	}

	b = b[len(constant.StrBytes):]
	if len(b) == 0 || b[0] != '=' {
...
func (h *fsHandler) compressAndOpenFSFile(filePath string) (*fsFile, error) {
	...
	if strings.HasSuffix(filePath, h.compressedFileSuffix) ||
		...
		!compress.IsFileCompressible(f, fsMinCompressRatio) {
