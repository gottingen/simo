...
func TimeoutWithCodeHandler(h RequestHandler, timeout time.Duration, msg string, statusCode int) RequestHandler {
	...
	return func(ctx *RequestCtx) {
		...
		ctx.timeoutTimer = timer.InitTimer(ctx.timeoutTimer, timeout)
		select {
		case <-ch:
		case <-ctx.timeoutTimer.C:
			ctx.TimeoutErrorWithCode(msg, statusCode)
		}
		timer.StopTimer(ctx.timeoutTimer)
	}
}
...
func (ctx *RequestCtx) Logger() *ctxLogger {
...
func (ctx *RequestCtx) Init2(conn net.Conn, logger *viper.Logger, reduceMemoryUsage bool) {
	ctx.c = conn
...
//
// This function is intended for custom Server implementations.
func (ctx *RequestCtx) Init(req *Request, remoteAddr net.Addr, logger *viper.Logger) {
	if remoteAddr == nil {
