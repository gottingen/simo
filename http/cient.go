...
func (c *Client) Do(req *Request, resp *Response) error {
	...
	isTLS := false
	scheme := uri.Scheme()
	if bytes.Equal(scheme, constant.StrHTTPS) {
		isTLS = true
	} else if !bytes.Equal(scheme, constant.StrHTTP) {
...
func clientPostURL(dst []byte, url string, postArgs *Args, c clientDoer) (statusCode int, body []byte, err error) {
	req := AcquireRequest()
	req.Header.SetMethodBytes(constant.StrPost)
	req.Header.SetContentTypeBytes(constant.StrPostArgsContentType)
	if postArgs != nil {
...
func doRequestFollowRedirects(req *Request, dst []byte, url string, c clientDoer) (statusCode int, body []byte, err error) {
	...
	for {
		...
		if redirectsCount > 0 && !bytes.Equal(scheme, req.uri.Scheme()) {
			if strings.HasPrefix(url, string(constant.StrHTTPS)) {
				req.isTLS = true
				req.uri.SetSchemeBytes(constant.StrHTTPS)
			} else {
				req.isTLS = false
				req.uri.SetSchemeBytes(constant.StrHTTP)
			}
			scheme = req.uri.Scheme()
		...
		location := resp.Header.peek(constant.StrLocation)
		if len(location) == 0 {
...
func StatusCodeIsRedirect(statusCode int) bool {
	return statusCode == constant.StatusMovedPermanently ||
		statusCode == constant.StatusFound ||
		statusCode == constant.StatusSeeOther ||
		statusCode == constant.StatusTemporaryRedirect ||
...
func (c *HostClient) doNonNilReqResp(req *Request, resp *Response) (bool, error) {
	...
	if req.schemaUpdate {
		c.IsTLS = bytes.Equal(req.URI().Scheme(), constant.StrHTTPS)
...
func (c *HostClient) getClientName() []byte {
	...
	if v == nil {
		...
		if len(clientName) == 0 && !c.NoDefaultUserAgentHeader {
			clientName = constant.DefaultUserAgent
		}
