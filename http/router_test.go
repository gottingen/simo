...
func TestRouter(t *testing.T) {
	...
	routed := false
	router.Handle("GET", "/user/:name", func(ctx *RequestCtx) {
		routed = true
...
func TestRouterAPI(t *testing.T) {
	...
	router := New()
	router.GET("/GET", func(ctx *RequestCtx) {
		get = true
	})
	router.HEAD("/GET", func(ctx *RequestCtx) {
		head = true
	})
	router.OPTIONS("/GET", func(ctx *RequestCtx) {
		options = true
	})
	router.POST("/POST", func(ctx *RequestCtx) {
		post = true
	})
	router.PUT("/PUT", func(ctx *RequestCtx) {
		put = true
	})
	router.PATCH("/PATCH", func(ctx *RequestCtx) {
		patch = true
	})
	router.DELETE("/DELETE", func(ctx *RequestCtx) {
		deleted = true
	})
...
func TestRouterChaining(t *testing.T) {
	...
	fooHit := false
	router1.POST("/foo", func(ctx *RequestCtx) {
		fooHit = true
		ctx.SetStatusCode(constant.StatusOK)
	})

	barHit := false
	router2.POST("/bar", func(ctx *RequestCtx) {
		barHit = true
		ctx.SetStatusCode(constant.StatusOK)
	})
	...
	if !(resp.Header.StatusCode() == constant.StatusOK && fooHit) {
	...
	if !(resp.Header.StatusCode() == constant.StatusOK && barHit) {
	...
	if !(resp.Header.StatusCode() == constant.StatusNotFound) {
...
func TestRouterOPTIONS(t *testing.T) {
	...
	// custom handler
	var custom bool
	router.OPTIONS("/path", func(_ *RequestCtx) {
		custom = true
	})
...
func TestRouterNotAllowed(t *testing.T) {
	...
	if !(resp.Header.StatusCode() == constant.StatusMethodNotAllowed) {
	...
	router.MethodNotAllowed = RequestHandler(func(ctx *RequestCtx) {
		ctx.SetStatusCode(constant.StatusTeapot)
	...
	if resp.Header.StatusCode() != constant.StatusTeapot {
		t.Errorf("unexpected response code %d want %d", resp.Header.StatusCode(), constant.StatusTeapot)
	}
...
func TestRouterNotFound(t *testing.T) {
	...
	var notFound bool
	router.NotFound = RequestHandler(func(ctx *RequestCtx) {
...
func TestRouterPanicHandler(t *testing.T) {
	...
	router.PanicHandler = func(ctx *RequestCtx, p interface{}) {
		panicHandled = true
	}

	router.Handle("PUT", "/user/:name", func(_ *RequestCtx) {
		panic("oops!")
	})
