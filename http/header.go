...
func (h *ResponseHeader) SetContentRange(startPos, endPos, contentLength int) {
	...
	h.SetCanonical(constant.StrContentRange, h.bufKV.value)
}
...
func (h *RequestHeader) SetByteRange(startPos, endPos int) {
	...
	h.SetCanonical(constant.StrRange, h.bufKV.value)
}

// StatusCode returns response status code.
func (h *ResponseHeader) StatusCode() int {
	if h.statusCode == 0 {
		return constant.StatusOK
	}
	return h.statusCode
}
...
func (h *ResponseHeader) SetLastModified(t time.Time) {
	h.bufKV.value = bytesconv.AppendHTTPDate(h.bufKV.value[:0], t)
	h.SetCanonical(constant.StrLastModified, h.bufKV.value)
}
...
func (h *ResponseHeader) ResetConnectionClose() {
	if h.connectionClose {
		...
		h.h = delAllArgsBytes(h.h, constant.StrConnection)
	}
}
...
func (h *ResponseHeader) ConnectionUpgrade() bool {
	return hasHeaderValue(h.Peek(constant.HeaderConnection), constant.StrUpgrade)
}
...
func (h *ResponseHeader) SetContentLength(contentLength int) {
	...
	if contentLength >= 0 {
		...
		h.h = delAllArgsBytes(h.h, constant.StrTransferEncoding)
	} else {
		h.contentLengthBytes = h.contentLengthBytes[:0]
		value := constant.StrChunked
		if contentLength == -2 {
			h.SetConnectionClose()
			value = constant.StrIdentity
		}
		h.h = setArgBytes(h.h, constant.StrTransferEncoding, value, argsHasValue)
	}
}
...
func (h *ResponseHeader) mustSkipContentLength() bool {
	...
	// Fast path.
	if statusCode < 100 || statusCode == constant.StatusOK {
		return false
	}

	// Slow path.
	return statusCode == constant.StatusNotModified || statusCode == constant.StatusNoContent || statusCode < 200
}
...
func (h *RequestHeader) SetContentLength(contentLength int) {
	...
	if contentLength >= 0 {
		...
		h.contentLengthBytes = h.contentLengthBytes[:0]
		h.h = setArgBytes(h.h, constant.StrTransferEncoding, constant.StrChunked, argsHasValue)
	}
}

func (h *ResponseHeader) isCompressibleContentType() bool {
	contentType := h.ContentType()
	return bytes.HasPrefix(contentType, constant.StrTextSlash) ||
		bytes.HasPrefix(contentType, constant.StrApplicationSlash)
}
...
func (h *ResponseHeader) ContentType() []byte {
	...
	if !h.noDefaultContentType && len(h.contentType) == 0 {
		contentType = constant.DefaultContentType
	}
	return contentType
}
...
func (h *RequestHeader) SetMultipartFormBoundary(boundary string) {
	...
	b := h.bufKV.value[:0]
	b = append(b, constant.StrMultipartFormData...)
	b = append(b, ';', ' ')
	b = append(b, constant.StrBoundary...)
	b = append(b, '=')
	b = append(b, boundary...)
	h.bufKV.value = b
...
func (h *RequestHeader) MultipartFormBoundary() []byte {
	b := h.ContentType()
	if !bytes.HasPrefix(b, constant.StrMultipartFormData) {
		return nil
	}
	b = b[len(constant.StrMultipartFormData):]
	if len(b) == 0 || b[0] != ';' {
		return nil
	}
	...
	for len(b) > 0 {
		...
		b = b[n:]
		if !bytes.HasPrefix(b, constant.StrBoundary) {
...
func (h *RequestHeader) Host() []byte {
	...
	if !h.rawHeadersParsed {
		...
		host := peekRawHeader(h.rawHeaders, constant.StrHost)
		if len(host) > 0 {
...
func (h *RequestHeader) IsGet() bool {
	return bytes.Equal(h.Method(), constant.StrGet)
}

// IsPost returns true if request method is POST.
func (h *RequestHeader) IsPost() bool {
	return bytes.Equal(h.Method(), constant.StrPost)
}

// IsPut returns true if request method is PUT.
func (h *RequestHeader) IsPut() bool {
	return bytes.Equal(h.Method(), constant.StrPut)
}

// IsHead returns true if request method is HEAD.
func (h *RequestHeader) IsHead() bool {
	return bytes.Equal(h.Method(), constant.StrHead)
}

// IsDelete returns true if request method is DELETE.
func (h *RequestHeader) IsDelete() bool {
	return bytes.Equal(h.Method(), constant.StrDelete)
}

// IsConnect returns true if request method is CONNECT.
func (h *RequestHeader) IsConnect() bool {
	return bytes.Equal(h.Method(), constant.StrConnect)
}

// IsOptions returns true if request method is OPTIONS.
func (h *RequestHeader) IsOptions() bool {
	return bytes.Equal(h.Method(), constant.StrOptions)
}

// IsTrace returns true if request method is TRACE.
func (h *RequestHeader) IsTrace() bool {
	return bytes.Equal(h.Method(), constant.StrTrace)
}

// IsPatch returns true if request method is PATCH.
func (h *RequestHeader) IsPatch() bool {
	return bytes.Equal(h.Method(), constant.StrPatch)
}
...
func (h *RequestHeader) HasAcceptEncodingBytes(acceptEncoding []byte) bool {
	ae := h.peek(constant.StrAcceptEncoding)
...
func (h *ResponseHeader) VisitAll(f func(key, value []byte)) {
	if len(h.contentLengthBytes) > 0 {
		f(constant.StrContentLength, h.contentLengthBytes)
	}
	contentType := h.ContentType()
	if len(contentType) > 0 {
		f(constant.StrContentType, contentType)
	}
	server := h.Server()
	if len(server) > 0 {
		f(constant.StrServer, server)
	}
	if len(h.cookies) > 0 {
		visitArgs(h.cookies, func(k, v []byte) {
			f(constant.StrSetCookie, v)
		})
	}
	visitArgs(h.h, f)
	if h.ConnectionClose() {
		f(constant.StrConnection, constant.StrClose)
	}
}
...
func (h *RequestHeader) VisitAll(f func(key, value []byte)) {
	...
	if len(userAgent) > 0 {
		f(constant.StrUserAgent, userAgent)
	}

	h.collectCookies()
	if len(h.cookies) > 0 {
		h.bufKV.value = appendRequestCookieBytes(h.bufKV.value[:0], h.cookies)
		f(constant.StrCookie, h.bufKV.value)
	}
	visitArgs(h.h, f)
	if h.ConnectionClose() {
		f(constant.StrConnection, constant.StrClose)
...
func (h *ResponseHeader) del(key []byte) {
	switch string(key) {
	case constant.HeaderContentType:
		h.contentType = h.contentType[:0]
	case constant.HeaderServer:
		h.server = h.server[:0]
	case constant.HeaderSetCookie:
		h.cookies = h.cookies[:0]
	case constant.HeaderContentLength:
		h.contentLength = 0
		h.contentLengthBytes = h.contentLengthBytes[:0]
	case constant.HeaderConnection:
		h.connectionClose = false
	}
...
func (h *RequestHeader) del(key []byte) {
	switch string(key) {
	case constant.HeaderHost:
		h.host = h.host[:0]
	case constant.HeaderContentType:
		h.contentType = h.contentType[:0]
	case constant.HeaderUserAgent:
		h.userAgent = h.userAgent[:0]
...
func (h *ResponseHeader) Add(key, value string) {
	k := getHeaderKeyBytes(&h.bufKV, key, h.disableNormalizing)
	h.h = appendArg(h.h, bytesconv.ToString(k), value, argsHasValue)
}
...
func (h *ResponseHeader) AddBytesK(key []byte, value string) {
	h.Add(bytesconv.ToString(key), value)
}
...
func (h *ResponseHeader) AddBytesV(key string, value []byte) {
	h.Add(key, bytesconv.ToString(value))
}
...
func (h *ResponseHeader) AddBytesKV(key, value []byte) {
	h.Add(bytesconv.ToString(key), bytesconv.ToString(value))
}
...
func (h *ResponseHeader) SetCanonical(key, value []byte) {
	...
	case constant.HeaderConnection:
		if bytes.Equal(constant.StrClose, value) {
			h.SetConnectionClose()
		} else {
			h.ResetConnectionClose()
			h.h = setArgBytes(h.h, key, value, argsHasValue)
		}
	case constant.HeaderTransferEncoding:
		// Transfer-Encoding is managed automatically.
	case constant.HeaderDate:
...
func (h *RequestHeader) SetCookieBytesK(key []byte, value string) {
	h.SetCookie(bytesconv.ToString(key), value)
}

// SetCookieBytesKV sets 'key: value' cookies.
func (h *RequestHeader) SetCookieBytesKV(key, value []byte) {
	h.SetCookie(bytesconv.ToString(key), bytesconv.ToString(value))
}
...
func (h *ResponseHeader) DelClientCookieBytes(key []byte) {
	h.DelClientCookie(bytesconv.ToString(key))
}
...
func (h *ResponseHeader) DelCookieBytes(key []byte) {
	h.DelCookie(bytesconv.ToString(key))
}
...
func (h *ResponseHeader) AppendBytes(dst []byte) []byte {
	...
	dst = append(dst, constant.StatusLine(statusCode)...)

	server := h.Server()
	if len(server) != 0 {
		dst = appendHeaderLine(dst, constant.StrServer, server)
	}

	serverDateOnce.Do(updateServerDate)
	dst = appendHeaderLine(dst, constant.StrDate, serverDate.Load().([]byte))
	...
	if h.ContentLength() != 0 || len(h.contentType) > 0 {
		contentType := h.ContentType()
		if len(contentType) > 0 {
			dst = appendHeaderLine(dst, constant.StrContentType, contentType)
		}
	}

	if len(h.contentLengthBytes) > 0 {
		dst = appendHeaderLine(dst, constant.StrContentLength, h.contentLengthBytes)
	}

	for i, n := 0, len(h.h); i < n; i++ {
		kv := &h.h[i]
		if !bytes.Equal(kv.key, constant.StrDate) {
	...
	if n > 0 {
		for i := 0; i < n; i++ {
			kv := &h.cookies[i]
			dst = appendHeaderLine(dst, constant.StrSetCookie, kv.value)
		}
	}

	if h.ConnectionClose() {
		dst = appendHeaderLine(dst, constant.StrConnection, constant.StrClose)
	}

	return append(dst, constant.StrCRLF...)
}
...
func (h *RequestHeader) AppendBytes(dst []byte) []byte {
	...
	dst = append(dst, ' ')
	dst = append(dst, constant.StrHTTP11...)
	dst = append(dst, constant.StrCRLF...)
	...
	userAgent := h.UserAgent()
	if len(userAgent) > 0 {
		dst = appendHeaderLine(dst, constant.StrUserAgent, userAgent)
	}

	host := h.Host()
	if len(host) > 0 {
		dst = appendHeaderLine(dst, constant.StrHost, host)
	}

	contentType := h.ContentType()
	if !h.ignoreBody() {
		if len(contentType) == 0 {
			contentType = constant.StrPostArgsContentType
		}
		dst = appendHeaderLine(dst, constant.StrContentType, contentType)
	...
	n := len(h.cookies)
	if n > 0 {
		dst = append(dst, constant.StrCookie...)
		dst = append(dst, constant.StrColonSpace...)
...
func (h *ResponseHeader) parseFirstLine(buf []byte) (int, error) {
	...
	h.noHTTP11 = !bytes.Equal(b[:n], constant.StrHTTP11)
	b = b[n+1:]

	// parse status code
	h.statusCode, n, err = bytesconv.ParseUintBuf(b)
	if err != nil {
...
func (h *RequestHeader) parseFirstLine(buf []byte) (int, error) {
	...
	} else if !bytes.Equal(b[n+1:], constant.StrHTTP11) {
		h.noHTTP11 = true
	}
...
func (h *ResponseHeader) parseHeaders(buf []byte) (int, error) {
	...
	for s.next() {
		if len(s.key) > 0 {
			switch s.key[0] | 0x20 {
			case 'c':
				if caseInsensitiveCompare(s.key, constant.StrContentType) {
					h.contentType = append(h.contentType[:0], s.value...)
					continue
				}
				if caseInsensitiveCompare(s.key, constant.StrContentLength) {
				...
				if caseInsensitiveCompare(s.key, constant.StrConnection) {
					if bytes.Equal(s.value, constant.StrClose) {
						h.connectionClose = true
					} else {
						h.connectionClose = false
			...
			case 's':
				if caseInsensitiveCompare(s.key, constant.StrServer) {
					h.server = append(h.server[:0], s.value...)
					continue
				}
				if caseInsensitiveCompare(s.key, constant.StrSetCookie) {
					h.cookies, kv = allocArg(h.cookies)
			...
			case 't':
				if caseInsensitiveCompare(s.key, constant.StrTransferEncoding) {
					if !bytes.Equal(s.value, constant.StrIdentity) {
						h.contentLength = -1
						h.h = setArgBytes(h.h, constant.StrTransferEncoding, constant.StrChunked, argsHasValue)
					}
					continue
	...
	if h.contentLength == -2 && !h.ConnectionUpgrade() && !h.mustSkipContentLength() {
		h.h = setArgBytes(h.h, constant.StrTransferEncoding, constant.StrIdentity, argsHasValue)
		h.connectionClose = true
	}
	if h.noHTTP11 && !h.connectionClose {
		// close connection for non-http/1.1 response unless 'Connection: keep-alive' is set.
		v := peekArgBytes(h.h, constant.StrConnection)
		h.connectionClose = !hasHeaderValue(v, constant.StrKeepAlive)
	}
...
func (h *RequestHeader) parseHeaders(buf []byte) (int, error) {
	...
	for s.next() {
		if len(s.key) > 0 {
			...
			switch s.key[0] | 0x20 {
			case 'h':
				if caseInsensitiveCompare(s.key, constant.StrHost) {
					h.host = append(h.host[:0], s.value...)
					continue
				}
			case 'u':
				if caseInsensitiveCompare(s.key, constant.StrUserAgent) {
...
func (h *RequestHeader) collectCookies() {
	...
	for i, n := 0, len(h.h); i < n; i++ {
		kv := &h.h[i]
		if bytes.Equal(kv.key, constant.StrCookie) {
...
func AppendNormalizedHeaderKeyBytes(dst, key []byte) []byte {
	return AppendNormalizedHeaderKey(dst, bytesconv.ToString(key))
}
