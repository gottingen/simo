...
func TestServeFileHead(t *testing.T) {
	...
	var ctx RequestCtx
	var req Request
	req.Header.SetMethod(constant.MethodHead)
...
func TestServeFileCompressed(t *testing.T) {
	...
	var req Request
	req.SetRequestURI("http://foobar.com/baz")
	req.Header.Set(constant.HeaderAcceptEncoding, "gzip")
	ctx.Init(&req, nil, nil)
...
func testFSByteRange(t *testing.T, h RequestHandler, filePath string) {
	...
	if resp.StatusCode() != constant.StatusPartialContent {
		t.Fatalf("unexpected status code: %d. Expecting %d. filePath=%q", resp.StatusCode(), constant.StatusPartialContent, filePath)
	}
	cr := resp.Header.Peek(constant.HeaderContentRange)
...
func testFSCompress(t *testing.T, h RequestHandler, filePath string) {
	...
	ctx.Request.Reset()
	ctx.Request.SetRequestURI(filePath)
	ctx.Request.Header.Set(constant.HeaderAcceptEncoding, "gzip")
	h(&ctx)
	s = ctx.Response.String()
	br = bufio.NewReader(bytes.NewBufferString(s))
	if err := resp.Read(br); err != nil {
...
func TestFileExtension(t *testing.T) {
	...
	testFileExtension(t, "foo.bar.baz.simo.gz", true, ".simo.gz", ".baz")
...
func TestServeFileContentType(t *testing.T) {
	...
	var ctx RequestCtx
	var req Request
	req.Header.SetMethod(constant.MethodGet)
	req.SetRequestURI("http://foobar.com/baz")
	ctx.Init(&req, nil, nil)
