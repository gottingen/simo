...
func TestServerInvalidHeader(t *testing.T) {
	...
	if resp.StatusCode() != constant.StatusBadRequest {
		t.Fatalf("unexpected status code: %d. Expecting %d", resp.StatusCode(), constant.StatusBadRequest)
	}
	...
	if resp.StatusCode() != constant.StatusOK {
		t.Fatalf("unexpected status code: %d. Expecting %d", resp.StatusCode(), constant.StatusOK)
	}
...
func TestServerConnState(t *testing.T) {
	...
	go func() {
		...
		for i := 0; i < 2; i++ {
			...
			if resp.StatusCode() != constant.StatusOK {
				t.Errorf("unexpected status code: %d. Expecting %d", resp.StatusCode(), constant.StatusOK)
			}
		}
		if err := c.Close(); err != nil {
			t.Errorf("unexpected error: %s", err)
...
func TestServerName(t *testing.T) {
	...
	resp := getReponse()
	if !bytes.Contains(resp, []byte("\r\nServer: "+string(constant.DefaultServerName)+"\r\n")) {
		t.Fatalf("Unexpected response %q expected Server: "+string(constant.DefaultServerName), resp)
	}

	// We can't just overwrite s.Name as http caches the name in an atomic.Value
	s = &Server{
		Handler: func(ctx *RequestCtx) {
		},
		Name: "foobar",
...
func TestServerErrSmallBuffer(t *testing.T) {
	...
	if statusCode != constant.StatusRequestHeaderFieldsTooLarge {
		t.Fatalf("unexpected status code: %d. Expecting %d", statusCode, constant.StatusRequestHeaderFieldsTooLarge)
	}
...
func TestRequestCtxRedirectHTTPSSchemeless(t *testing.T) {
	...
	ctx.Redirect("//foobar.com/aa/bbb", constant.StatusFound)
	location := ctx.Response.Header.Peek(constant.HeaderLocation)
...
func testRequestCtxRedirect(t *testing.T, origURL, redirectURL, expectedURL string) {
	...
	ctx.Redirect(redirectURL, constant.StatusFound)
	loc := ctx.Response.Header.Peek(constant.HeaderLocation)
...
func TestServerResponseServerHeader(t *testing.T) {
	...
	go func() {
		...
		if resp.StatusCode() != constant.StatusNotFound {
			t.Errorf("unexpected status code: %d. Expecting %d", resp.StatusCode(), constant.StatusNotFound)
		}
...
func TestServerResponseBodyStream(t *testing.T) {
	...
	go func() {
		...
		if respH.StatusCode() != constant.StatusOK {
			t.Errorf("unexpected status code: %d. Expecting %d", respH.StatusCode(), constant.StatusOK)
		}
...
func TestServerMaxConnsPerIPLimit(t *testing.T) {
	...
	go func() {
		...
		if resp.StatusCode() != constant.StatusTooManyRequests {
			t.Errorf("unexpected status code for the second connection: %d. Expecting %d",
				resp.StatusCode(), constant.StatusTooManyRequests)
		}
		...
		if resp.StatusCode() != constant.StatusOK {
			t.Errorf("unexpected status code for the first connection: %d. Expecting %d",
				resp.StatusCode(), constant.StatusOK)
		}
		if string(resp.Body()) != "OK" {
...
func TestServerConcurrencyLimit(t *testing.T) {
	...
	go func() {
		...
		if resp.StatusCode() != constant.StatusServiceUnavailable {
			t.Errorf("unexpected status code for the second connection: %d. Expecting %d",
				resp.StatusCode(), constant.StatusServiceUnavailable)
		}
...
func TestServerWriteFastError(t *testing.T) {
	...
	s.writeFastError(&buf, constant.StatusForbidden, expectedBody)
	...
	if resp.StatusCode() != constant.StatusForbidden {
		t.Fatalf("unexpected status code: %d. Expecting %d", resp.StatusCode(), constant.StatusForbidden)
	}
	body := resp.Body()
...
func TestServerServeTLSEmbed(t *testing.T) {
	...
	go func() {
		err := ServeTLSEmbed(ln, certData, keyData, func(ctx *RequestCtx) {
			if !ctx.IsTLS() {
				ctx.Error("expecting tls", constant.StatusBadRequest)
				return
			}
			scheme := ctx.URI().Scheme()
			if string(scheme) != "https" {
				ctx.Error(fmt.Sprintf("unexpected scheme=%q. Expecting %q", scheme, "https"), constant.StatusBadRequest)
				return
			}
...
`
	...
	s := &Server{
		Handler: func(ctx *RequestCtx) {
			...
			case "/upload":
				...
				ctx.Redirect("/", constant.StatusSeeOther)
			default:
	...
	go func() {
		...
		if resp.StatusCode() != constant.StatusSeeOther {
			t.Errorf("unexpected status code %d. Expecting %d", resp.StatusCode(), constant.StatusSeeOther)
		}
		loc := resp.Header.Peek(constant.HeaderLocation)
...
func testServerRequests(t *testing.T, ln *listener.MemListener) {
	conn, err := ln.Dial()
	if err != nil {
		t.Fatalf("unexpected error: %s", err)
	}
...
func TestServerHTTP10ConnectionClose(t *testing.T) {
	...
	go func() {
		err := Serve(ln, func(ctx *RequestCtx) {
			...
			ctx.Request.Header.Set(constant.HeaderConnection, "keep-alive")
			ctx.Response.Header.ResetConnectionClose()
			ctx.Response.Header.Set(constant.HeaderConnection, "keep-alive")
		})
		if err != nil {
			t.Errorf("unexpected error: %s", err)
		}
...
func TestServerHeadRequest(t *testing.T) {
	...
	if resp.Header.StatusCode() != constant.StatusOK {
		t.Fatalf("unexpected status code: %d. Expecting %d", resp.Header.StatusCode(), constant.StatusOK)
	}
	if len(resp.Body()) > 0 {
...
func TestServerExpect100Continue(t *testing.T) {
	...
	br := bufio.NewReader(&rw.w)
	verifyResponse(t, br, constant.StatusOK, string(constant.DefaultContentType), "foobar")
...
func TestCompressHandler(t *testing.T) {
	...
	expectedBody := string(util.CreateFixedBytes(2e4))
	...
	ctx.Request.Reset()
	ctx.Response.Reset()
	ctx.Request.Header.Set(constant.HeaderAcceptEncoding, "foobar, deflate, sdhc")
...
func TestServeConnNonHTTP11KeepAlive(t *testing.T) {
	...
	if string(resp.Header.Peek(constant.HeaderConnection)) != "keep-alive" {
		t.Fatalf("unexpected Connection header %q. Expecting %q", resp.Header.Peek(constant.HeaderConnection), "keep-alive")
	}
	if resp.Header.ConnectionClose() {
	...
	if string(resp.Header.Peek(constant.HeaderConnection)) != "close" {
		t.Fatalf("unexpected Connection header %q. Expecting %q", resp.Header.Peek(constant.HeaderConnection), "close")
	}
	if !resp.Header.ConnectionClose() {
...
func TestRequestCtxIfModifiedSince(t *testing.T) {
	...
	ctx.Request.Header.Set("If-Modified-Since", string(bytesconv.AppendHTTPDate(nil, lastModified)))
...
func TestRequestCtxSendFileNotModified(t *testing.T) {
	...
	if resp.StatusCode() != constant.StatusNotModified {
		t.Fatalf("unexpected status code: %d. Expecting %d", resp.StatusCode(), constant.StatusNotModified)
	}
	if len(resp.Body()) > 0 {
...
func TestRequestCtxHijack(t *testing.T) {
	...
	br := bufio.NewReader(&rw.w)
	verifyResponse(t, br, constant.StatusOK, "foo/bar", "hijack it!")
...
func TestTimeoutHandlerSuccess(t *testing.T) {
	...
	for i := 0; i < concurrency; i++ {
		go func() {
			...
			br := bufio.NewReader(conn)
			verifyResponse(t, br, constant.StatusOK, "aaa/bbb", "real response")
			clientCh <- struct{}{}
		}()
	}
...
func TestTimeoutHandlerTimeout(t *testing.T) {
	...
	for i := 0; i < concurrency; i++ {
		go func() {
			...
			br := bufio.NewReader(conn)
			verifyResponse(t, br, constant.StatusRequestTimeout, string(constant.DefaultContentType), "timeout!!!")
			clientCh <- struct{}{}
		}()
	}
...
func TestServerGetOnly(t *testing.T) {
	...
	statusCode := resp.StatusCode()
	if statusCode != constant.StatusBadRequest {
		t.Fatalf("unexpected status code: %d. Expecting %d", statusCode, constant.StatusBadRequest)
	}
	if !resp.ConnectionClose() {
		t.Fatal("missing 'Connection: close' response header")
	}
...
func TestServerTimeoutErrorWithCode(t *testing.T) {
	...
	s := &Server{
		Handler: func(ctx *RequestCtx) {
			...
			ctx.TimeoutErrorWithCode("stolen ctx", constant.StatusBadRequest)
		},
	}
	...
	br := bufio.NewReader(&rw.w)
	verifyResponse(t, br, constant.StatusBadRequest, string(constant.DefaultContentType), "stolen ctx")
...
func TestServerTimeoutError(t *testing.T) {
	...
	br := bufio.NewReader(&rw.w)
	verifyResponse(t, br, constant.StatusRequestTimeout, string(constant.DefaultContentType), "stolen ctx")
...
func TestServerMaxRequestsPerConn(t *testing.T) {
	...
	verifyResponseHeader(t, &resp.Header, 200, 0, string(constant.DefaultContentType))
...
func TestServerRequestNumAndTime(t *testing.T) {
	...
	br := bufio.NewReader(&rw.w)
	verifyResponse(t, br, 200, string(constant.DefaultContentType), "")
}
...
func TestServerConnError(t *testing.T) {
	...
	if !bytes.Equal(resp.Header.Peek(constant.HeaderContentType), constant.DefaultContentType) {
		t.Fatalf("Unexpected Content-Type %q. Expected %q", resp.Header.Peek(constant.HeaderContentType),constant.DefaultContentType)
	}
...
func TestServeConnSingleRequest(t *testing.T) {
	...
	s := &Server{
		Handler: func(ctx *RequestCtx) {
			h := &ctx.Request.Header
			ctx.Success("aaa", []byte(fmt.Sprintf("requestURI=%s, host=%s", h.RequestURI(), h.Peek(constant.HeaderHost))))
		},
	}
