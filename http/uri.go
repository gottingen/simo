...
func (u *URI) parse(host, uri []byte, isTLS bool) {
	...
	bytesconv.LowercaseBytes(u.scheme)
	if isTLS {
		u.scheme = append(u.scheme[:0], constant.StrHTTPS...)
	}

	if n := bytes.Index(host, constant.StrAt); n >= 0 {
		auth := host[:n]
		host = host[n+1:]

		if n := bytes.Index(auth, constant.StrColon); n >= 0 {
			u.username = auth[:n]
			u.password = auth[n+1:]
		} else {
			u.username = auth
...
func normalizePath(dst, src []byte) []byte {
	...
	dst = bytesconv.DecodeArgAppendNoPlus(dst, src)

	// remove duplicate slashes
	b := dst
	bSize := len(b)
	for {
		n := bytes.Index(b, constant.StrSlashSlash)
		if n < 0 {
			break
		}
		b = b[n:]
	...
	b = dst
	for {
		n := bytes.Index(b, constant.StrSlashDotSlash)
		if n < 0 {
			break
		}
		nn := n + len(constant.StrSlashDotSlash) - 1
		copy(b[n:], b[nn:])
		b = b[:len(b)-nn+n]
	}
	...
	// remove /foo/../ parts
	for {
		n := bytes.Index(b, constant.StrSlashDotDotSlash)
		if n < 0 {
			break
		}
		...
		n += len(constant.StrSlashDotDotSlash) - 1
		copy(b[nn:], b[n:])
		b = b[:len(b)-n+nn]
	}

	// remove trailing /foo/..
	n := bytes.LastIndex(b, constant.StrSlashDotDot)
	if n >= 0 && n+len(constant.StrSlashDotDot) == len(b) {
		nn := bytes.LastIndexByte(b[:n], '/')
		if nn < 0 {
			return constant.StrSlash
		}
		b = b[:nn+1]
	}
...
func (u *URI) Update(newURI string) {
	u.UpdateBytes(bytesconv.Tobytes(newURI))
}
...
func (u *URI) updateBytes(newURI, buf []byte) []byte {
	...
	n := bytes.Index(newURI, constant.StrSlashSlash)
	if n >= 0 {
		// absolute uri
		var b [32]byte
		schemeOriginal := b[:0]
...
func (u *URI) appendSchemeHost(dst []byte) []byte {
	...
	dst = append(dst, constant.StrColonSlashSlash...)
...
func splitHostURI(host, uri []byte) ([]byte, []byte, []byte) {
	n := bytes.Index(uri, constant.StrSlashSlash)
	if n < 0 {
		return constant.StrHTTP, host, uri
	}
	scheme := uri[:n]
	...
	if n < 0 {
		...
		return scheme, uri, constant.StrSlash
	}
	return scheme, uri[:n], uri[n:]
}
